...
from pylib.perfutils import *  # noqa: F401,F403
...
def curl_ok(url: str) -> bool:
    return subprocess.run(
        ["curl", "-ks", url], capture_output=True, timeout=15,
    ).returncode == 0
...
def main() -> int:
    ...
    def cleanup() -> None:
        ...
        if perf_proc and perf_proc.poll() is None:
            subprocess.run([*sudo_perf, "kill", "-SIGINT", str(perf_proc.pid)],
        ...
        for sp in perf_stat_procs:
            if sp.poll() is None:
                subprocess.run([*sudo_perf, "kill", "-SIGINT", str(sp.pid)],
                               stderr=subprocess.DEVNULL)
                try:
                    sp.wait(timeout=10)
                except Exception:
    ...
    try:
        # coolwsd
        if curl_ok(f"{coolwsd_url}hosting/discovery"):
            ...
            wait_for("coolwsd ready", 60,
                     lambda: curl_ok(f"{coolwsd_url}hosting/discovery"))
        ...
        # WOPI host
        if curl_ok(f"{wopi_base}/wopi/files/1"):
            ...
            wait_for("WOPI host ready", 30,
                     lambda: curl_ok(f"{wopi_base}/wopi/files/1"))
        ...
        if runner == "k6":
            dialed = re.sub(r"//(localhost|127\.0\.0\.1)([:/])",
                            f"//{host_ip}\\2", coolwsd_url)
            if not curl_ok(f"{dialed}hosting/discovery"):
        ...
        def launch_and_wait_for_kit() -> tuple[float, subprocess.Popen, list[str]]:
            ...
            for i in range(30):
                ...
                if i % 3 == 2 and not curl_ok(f"{coolwsd_url}hosting/discovery"):
        ...
        if not kit_pids and not curl_ok(f"{coolwsd_url}hosting/discovery"):
